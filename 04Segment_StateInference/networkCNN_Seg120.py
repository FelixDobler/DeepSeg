...
def classifier(inp, is_training, init=False, reuse=False, getter=None, category=125):
    with tf.variable_scope("discriminator_model", reuse=reuse, custom_getter=getter):
        ...
        # x = tf.reshape(inp, [-1, 120, 30, 3]) # DeepSeg
        x = tf.reshape(inp, [-1, 60, 30, 3])  # Adjusted
        print("tf shapes:")
        print(x.shape)
        ...
        x = nn.conv2d(x, 96, nonlinearity=leakyReLu, init=init, counters=counter)
        print(x.shape)
        # Different strides needed to produce compatible shapes, [4,2] in DeepSeg, [2,2] in this code
        x = nn.conv2d(
        ...
        x = nn.conv2d(x, 192, nonlinearity=leakyReLu, init=init, counters=counter)
        print(x.shape)
        # Different strides needed to produce compatible shapes, [4,2] in DeepSeg, [4,2] in this code
        x = nn.conv2d(
            x, 192, stride=[4, 2], nonlinearity=leakyReLu, init=init, counters=counter
        )
        ...
        x = nn.nin(x, 192, counters=counter, nonlinearity=leakyReLu, init=init)
        print(x.shape)
        # Different pool sizes possible to adjust for shape differences due to window size and strides
        x = keras.layers.MaxPooling2D(pool_size=(6, 6), strides=1)(x)
        print(x.shape)
        x = tf.squeeze(x, [1, 2])
