...
def discretized_mix_logistic_loss(x, l, sum_all=True):
    ...
    means = tf.concat(
        3, [tf.reshape(means[:, :, :, 0, :], [xs[0], xs[1], xs[2], 1, nr_mix]), m2, m3]
    )
    centered_x = x - means
    ...
    with tf.variable_scope(name):
        if init:
            ...
            g = tf.get_variable(
                "g",
                dtype=tf.float32,
                initializer=tf.constant(np.ones(num_units), tf.float32),
                trainable=train_scale,
            )
            b = tf.get_variable(
                "b",
                dtype=tf.float32,
                initializer=tf.constant(np.zeros(num_units), tf.float32),
                trainable=True,
            )
    ...
    with tf.variable_scope(name):
        if init:
            ...
            g = tf.get_variable(
                "g",
                dtype=tf.float32,
                initializer=tf.constant(np.ones(num_filters), tf.float32),
                trainable=True,
            )
            b = tf.get_variable(
                "b",
                dtype=tf.float32,
                initializer=tf.constant(np.zeros(num_filters), tf.float32),
                trainable=True,
            )
            # print(b)
