...
parser.add_argument(
    "--dataDir", dest="dataDir", default="data", help="directory of data"
)
parser.add_argument(
    "--trainCsi",
    dest="trainCsi",
    default="segmentTrainCsi.mat",
    help="CSI data File For Train",
)
parser.add_argument(
    "--trainLab",
    dest="trainLab",
    default="segmentTrainLab.mat",
    help="Label data File For Train",
)
parser.add_argument(
    "--testCsi",
    dest="testCsi",
    default="segmentTestCsi.mat",
    help="CSI data File For Test",
)
parser.add_argument(
    "--testLab",
    dest="testLab",
    default="segmentTestLab.mat",
    help="Label data File For Test",
)
parser.add_argument(
    "--lr",
    dest="learning_rate",
    type=float,
    default=0.006,
    help="initial learning rate for adam [0.006]",
)
parser.add_argument(
    "--wtrain",
    dest="wtrain",
    type=bool,
    default=False,
    help="if input any values, this will be true",
)
...
flags.DEFINE_integer(
    "gpu", 0, "gpu [0]"
)  # which GPU is used. If it is beyong the number of GPU, CPU will is used.
flags.DEFINE_integer(
    "batch_size", 16, "batch size [25]"
)  # --------60:0.8417(1169)-------70:0.8597--------------
flags.DEFINE_integer(
    "category_number", 10, "number of categories in the dataset [125]"
)  # ---categoryNum = 125----
...
flags.DEFINE_float(
    "ma_decay", 0.9999, "exponential moving average for inference [0.9999]"
)
...
flags.DEFINE_float("eta", 1.0, "perturbation latent code")
...
def linear_decay(decay_start, decay_end, epoch):
    return min(
        -1 / (decay_end - decay_start) * epoch
        + 1
        + decay_start / (decay_end - decay_start),
        1,
    )
...
def mat2Npy(data_dir, fileName, typeName):
    ...
    fileName = list(mat.keys())[
        0
    ]  # print(list(mat.keys())[0]) #fileName = fileName.replace('.mat','')
    data = mat[fileName]
...
def main(_):
    ...
    (trainx, trainy, testx, testy) = loadData(
        args.dataDir, args.trainCsi, args.trainLab, args.testCsi, args.testLab
    )
    rng_data = np.random.RandomState(rng.randint(0, 2**10))  # seed shuffling
    inds = rng_data.permutation(
        trainx.shape[0]
    )  # -----------------xiao--------shuffling data-------------
    trainx = trainx[inds]
    trainy = trainy[inds]
    ...
    if (
        args.wtrain
    ):  # --------------------------add for sample Weight-------20191214------xiao------------
    ...
    print(
        "args.wtrain=%s, args.learning_rate=%.4f, args.trainCsi=%s"
        % (args.wtrain, args.learning_rate, args.trainCsi)
    )

    print("Data:")
    print(
        "train examples %d, batch %d, test examples %d, batch %d"
    ...
    inp = tf.placeholder(
        tf.float32,
        [FLAGS.batch_size, trainx.shape[1], 30, 3],
    ...
    classifier(
        inp, is_training_pl, init=True, category=FLAGS.category_number
    )  # initiate classifier

    logits_lab, layer_label = classifier(
        inp, is_training_pl, init=False, reuse=True, category=FLAGS.category_number
    )  # labeled_data_input_pl
    tf.add_to_collection(
        "logits_lab_save", logits_lab
    )  # ------------------add for save and load model-------20191127------xiao------------
    tf.add_to_collection(
        "batch_size_save", FLAGS.batch_size
    )  # ------------------add for save and load model-------20191127------xiao------------
    correct_pred = tf.equal(
        tf.cast(tf.argmax(logits_lab, 1), tf.int32), tf.cast(lbl, tf.int32)
    )
    ...
    with tf.name_scope("loss_functions"):
        ...
        if (
            ...
            sampleWeight = tf.placeholder(
                tf.float32, [FLAGS.batch_size], name="weight_for_loss"
            )
            loss_lab = tf.reduce_mean(
                tf.multiply(
                    sampleWeight,
                    tf.nn.sparse_softmax_cross_entropy_with_logits(
                        labels=lbl, logits=logits_lab
                    ),
                )
            )
        else:
        ...
        loss_class = (
            FLAGS.lbl_weight * loss_lab
        )  # ------------consider manifold to improve performance-------------------
    ...
    with tf.name_scope("optimizers"):
        ...
        optimizer_class = tf.train.AdamOptimizer(
            learning_rate=lr_pl, beta1=0.5, name="class_optimizer"
        )
        ...
        logits_ema, _ = classifier(
            inp,
            is_training_pl,
            getter=get_getter(ema),
            reuse=True,
            category=FLAGS.category_number,
        )
        tf.add_to_collection(
            "logits_ema_save", logits_ema
        )  # ------------------add for save and load model-------20191127------xiao------------
        correct_pred_ema = tf.equal(
            tf.cast(tf.argmax(logits_ema, 1), tf.int32), tf.cast(lbl, tf.int32)
        )
    ...
    saveModel = tf.train.Saver(
        max_to_keep=2000
    )  # ----------to overcome the thing that Saver has 5 models limit--------------------
    ...
    with sv.managed_session() as sess:  # tf.train.Supervisor----------------mainly used for saving model-------------xiao-------------
        ...
        print(
            "\ninitialization done"
        )  # tf.train.Supervisor-------https://blog.csdn.net/qq_37008037/article/details/86236808
        print(
            "Starting training from epoch :%d, step:%d \n"
        ...
        while not sv.should_stop():
            ...
            for t in range(nr_batches_train):
                ...
                _, acc, lb = sess.run(
                    [train_class_op, accuracy_classifier, loss_lab], feed_dict=feed_dict
                )
            ...
            if (epoch % FLAGS.freq_test == 0) | (epoch == FLAGS.epoch - 1):
                for t in range(nr_batches_test):
                    ...
                    acc, acc_ema, y_pred, y_pred_ema = sess.run(
                        [accuracy_classifier, accuracy_ema, y_predict, y_predict_ema],
                        feed_dict=feed_dict,
                    )
                    ...
                    f1_ema = metrics.f1_score(
                        y_true, y_pred_ema, average="weighted"
                ...
                if accMax_ema[0] < test_acc_ema:
                    ...
                    sess.run(
                        maxAccAssign_ema,
                        feed_dict={accTemp_ema: [test_acc_ema, f1All_ema, epoch]},
                    )

                print(
                    "Ep%d %ds lossLab=%.4f trainAcc=%.4f testAcc=%.2f maxAcc=%0.2f Ep%d testAccE=%0.2f testF1E=%0.2f maxAccE=%0.2f maxF1E=%0.2f Ep%d"
                    % (
                        epoch,
                        time.time() - begin,
                        train_loss_lab,
